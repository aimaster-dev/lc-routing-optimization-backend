...
async def upload_csv(file: UploadFile = File(...), landfill_file: UploadFile = File(...)):
    """
    Uploads a single CSV file, processes it, and returns the extracted data.
    """
    # Validate that the file is a CSV
    print(landfill_file.filename)
    if not file.filename.endswith(".csv") or not landfill_file.filename.endswith(".csv"):
        raise HTTPException(status_code=400, detail=f"File '{file.filename}' is not a CSV file.")

    try:
        # Save the file and process it
        file_path = save_file(file)
        landfill_file_path = save_file(landfill_file)
...
async def get_route_details_formatted(route_number: str):
    ...
    try:
        ...
        route_location_id = route_number.rsplit("-", 1)[0]
        print(route_location_id)
        details = get_details_by_route_formatted(file_name, route_location_id)
        return {
...
async def download_csv():
    # print(123123123)
    await make_data_for_download(1)
